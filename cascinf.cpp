...
// Xiaoyan
void TNIBs::PrefixCascade(TCascade& C, TIntFltH& PrefxInfectedNIdH) {
	//bool verbose = false;
	bool verbose = true;
	double startTm = C.BegI().GetDat().Tm;
	if (verbose)
		printf("prefix casc %d len %d startTm %f\n", C.GetId(), C.Len(), startTm);
	for (THash<TInt, THitInfo>::TIter NI = C.BegI(); NI < C.EndI(); NI++) {
		int node = NI.GetKey();
		double time = NI.GetDat().Tm;
		if (verbose)
			printf("\tnode %d time %f\n", node, time);
		if (time - startTm < 2.5) {
			PrefxInfectedNIdH.AddDat(node) = time;
		}
	}
	for (THash<TInt, TFlt>::TIter NI = PrefxInfectedNIdH.BegI(); NI < PrefxInfectedNIdH.EndI(); NI++){
		int node = NI.GetKey();
		double time = NI.GetDat();
		printf("\tread prefix node %d time %f\n", node, time);
	}
}

// Xiaoyan
double TNIBs::F1Score(TCascade& C1, TCascade& C2) const {
	int common = 0; 
	for (THash<TInt, THitInfo>::TIter NI = C1.BegI(); NI < C1.EndI(); NI++) {
		int node = NI.GetKey();
		//double time = NI.GetDat().Tm;
		if ( C2.IsNode(node) ) {
			++common;
	...
	double prec = (double)(common) / C1.Len();
	double recall = (double)(common) / C2.Len();
...
// Xiaoyan
void TNIBs::ReGenCascade(TCascade& C, TIntFltH& PrefxInfectedNIdH) {
	bool verbose = false;
	//bool verbose = true;
	...
	if (Network.GetNodes() == 0)
        ...
        // set random seed
        TInt::Rnd.Randomize();
	printf("new cascade %d\n", C.GetId());


	int counter = 0; // number of execution of the loop

	while (C.Len() < 2) {
		++counter;
		if (counter > 20) return; // 20 trials but got nothing
		C.Clr();
		InfectedNIdH.Clr();
		InfectedBy.Clr();

		// load prefix
		double prefixMaxTm = 0; //the maximum time of the 
		for (THash<TInt, TFlt>::TIter NI = PrefxInfectedNIdH.BegI(); NI < PrefxInfectedNIdH.EndI(); NI++){
			int node = NI.GetKey();
			//int node = Network.GetRndNId();
			double time = NI.GetDat();
			//float time = TFlt::Rnd.GetUniDev() * TotalTime; // random starting point <TotalTime
			InfectedNIdH.AddDat(node) = time;
			//InfectedNIdH.AddDat(node) = time;
			printf("\tload prefix node %d time %f\n", node, time);
			//break;
			//printf("a");
			if (time > prefixMaxTm) prefixMaxTm = time;
		}

		GlobalTime = PrefxInfectedNIdH.BegI().GetDat();
		InitTime = GlobalTime;
		//StartNId = Network.GetRndNId();
		StartNId = PrefxInfectedNIdH.BegI().GetKey();
		if (verbose) printf("Start node %d at time %.3f\n", StartNId, GlobalTime);
		//InfectedNIdH.AddDat(StartNId) = InitTime;
		...
		while (true) {
			...
			if (true) { printf("GlobalTime:%f, infected node:%d\n", GlobalTime, NId); }
			...
			for (int e = 0; e < NI.GetOutDeg(); e++) {
				...
				if (verbose)
				{ 
					printf("GlobalTime:%f, nodes:%d->%d, alpha:%f\n", GlobalTime, NId, DstNId, alpha);
					//TStrFltFltHNEDNet::TNodeI DstNI = Network.GetNI(DstNId);
					//printf("Node %d, deg %d\n", DstNId, DstNI.GetOutDeg());
	 			}
				...
				if (verbose)
					printf("sigma %f\t", sigmaT);
				IAssert(sigmaT >= 0);

				double t1 = TFlt::GetMn(GlobalTime + sigmaT, TFlt::GetMn(InitTime+Window, TotalTime));

				//the time has already proceeded to the current time, prefixMaxTm
				if ( t1 < prefixMaxTm) {
					if (verbose)
						printf("proceed to current time %f, ignore infection at time %f\n", prefixMaxTm, t1);
					continue;
				}
				...
				if (InfectedNIdH.IsKey(DstNId)) {
					...
					if ( t2 > t1 && t2 < TFlt::GetMn(InitTime+Window, TotalTime)) {
						...
						if (verbose)
							printf("update time: node %d from t2 %f-->t1 %f\n", DstNId, t2, t1);
						InfectedBy.GetDat(DstNId) = NId;
					}
					if (verbose) printf("do nothing\n");
				} else {
					if (verbose) printf("add: node %d at time %f\n", DstNId, t1);
...
void TNIBs::GenCascade(TCascade& C) {
	...
	while (C.Len() < 2) {
		...
		GlobalTime = InitTime;
		StartNId = Network.GetRndNId();
		printf("Start node %d at time %.3f\n", StartNId, GlobalTime);
		InfectedNIdH.AddDat(StartNId) = InitTime;
