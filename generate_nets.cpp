...
int main(int argc, char* argv[]) {
  ...
  const TStr GroundTruthFileName = Env.GetIfArgPrefixStr("-in:", TStr("input"), "Name of the input network (default:input)\n");  // input groundtruth, if any
	...
	//xiaoyan: force the program to read cascades
  const int TCasc = Env.GetIfArgPrefixInt("-tcasc:", 0, "Network to generate\n\
	  0:generate, 1:given in txt (default:0)\n");
  const TStr CascFileName = Env.GetIfArgPrefixStr("-incasc:", TStr("inputcasc"), "Name of the input cascades (default:inputcasc)\n");  // input groundtruth, if any
  ...
  if (TCasc<1) {
	  ...
	  NIBs.SaveCascades(TStr::Fmt("%s-cascades.txt", FileName.CStr()));
		...
		// load and validate the cascasdes
		printf("load network from %s\n", CascFileName.CStr());
		TFIn GFIn(CascFileName);		// open cascades file
		NIBs.LoadCascadesTxt(GFIn); // load cascades

  	for (int i = 0; i < NIBs.CascH.Len(); i++) {
			TCascade C(NIBs.CascH.Len() + i, NIBs.Model);
			TIntFltH PrefxInfectedNIdH;
  		NIBs.PrefixCascade(NIBs.CascH[i], PrefxInfectedNIdH);
		  printf("cascade:%d (%d nodes, first infection:%f, last infection:%f)\n", NIBs.CascH[i].GetId(), NIBs.CascH[i].Len(), NIBs.CascH[i].GetMinTm(), NIBs.CascH[i].GetMaxTm());

			NIBs.ReGenCascade(C, PrefxInfectedNIdH);
			if (C.Len() > 0) { 
			  printf("cascade:%d (%d nodes, first infection:%f, last infection:%f)\n", C.GetId(), C.Len(), C.GetMinTm(), C.GetMaxTm());
				NIBs.F1Score(NIBs.CascH[i], C);
			}
			else printf("regen cascade failed\n");
		}
  }
